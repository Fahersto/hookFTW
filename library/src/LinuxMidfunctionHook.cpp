...
namespace hookftw
{
	 ...
	 */
	void MidfunctionHook::ApplyHook(int8_t* sourceAddress, int hookLength, std::vector<int8_t> relocatedBytes, void proxy(context* ctx))
	{
		...
		*(int64_t*)&trampoline_[restoreRspAddress] = (int64_t)&originalRsp_;
		...
		Memory::ModifyPageProtection(sourceAddress, hookLength, MemoryPageProtection::HOOKFTW_PAGE_EXECUTE_READWRITE);
