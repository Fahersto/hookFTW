...
#if _WIN64
	void FuncStartHook::GenerateTrampolineAndApplyHook(int8_t* sourceAddress, int hookLength, std::vector<int8_t> relocatedBytes, void __fastcall proxy(context* ctx))
	{
...
void FuncStartHook::GenerateTrampolineAndApplyHook(int8_t* sourceAddress, int hookLength, std::vector<int8_t> relocatedBytes, void __fastcall proxy(context* ctx))
{
	...
	*(int32_t*)&sourceAddress[1] = (int32_t)(trampoline_ - sourceAddress) - jmpStubLength;
	 ...
	 */
	FuncStartHook::FuncStartHook(int8_t* sourceAddress, void __fastcall proxy(context* ctx))
		: savedRax_(0)
	{
		Decoder decoder;
